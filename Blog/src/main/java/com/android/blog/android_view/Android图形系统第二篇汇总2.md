...
> - Mac端可以使用SwitchResX软件来修改屏幕刷新率，这个方案我自己没试过（写篇文章而已不至于搭上台电脑），万一黑屏报废了就不好玩了
...
所以，发生画面撕裂的根本原因是：帧缓存的交换随时可能会发生，一旦显卡在不恰当的时机交换了帧缓存，屏幕就会发生画面撕裂
...
谷歌为了改进Android系统的流畅度，4.1版本发布的Project Butter（黄油计划）中，就是将图形更新策略改为垂直同步信号+三重缓存的方案

请注意，在Windows系统下运行的游戏绝大多数都是使用DirectX框架进行开发，DX游戏中的“三重缓存”和这里的三重缓存不是一个意思，DX中的那个是先进先出的帧缓存队列

显卡绘制完之后将帧数据存入缓存队列，当显卡接受到垂直同步信号之后把最顶端的帧数据交给屏幕显示

因此，PC游戏中开启“三重缓存”选项以后，最新的画面帧永远都在队列的最底端，这也是为什么PC端能够单独启用“三重缓冲”选项以及打开“垂直同步”选项后会感觉延迟变高的原因
...
对于竞技类游戏来说，建议不要开，不如买一块高刷屏来的爽快

高刷屏能解决画面撕裂吗？显然不能，但是高刷屏每个像素停留在屏幕上的时间变短了，即使发生画面撕裂，人眼也很难察觉到
