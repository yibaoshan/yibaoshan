...
切换帧缓存的动作通常是由显卡完成的，在2.2小节我们介绍LCD刷新原理时提到了，显示器并不是一次性更新所有的像素点，而是一行像素或者多行像素一起更新
...
- **方案二**
  ...
  > 一个巴掌拍不响，画面撕裂最主要的原因还是在显卡身上，显卡交换fram e buffer前也不问问显示器的意见
  >
  > 不过这也不能全怪显卡，毕竟它俩之间是没有通信的
  >
  > 让显示器每次刷新完喊一嗓子告诉显卡我好了，你可以干活了不就好了
  >
  > 哎，这一嗓子就是大名鼎鼎的VSync信号
  >
  > 

屏幕画面每隔一段时间就会刷新一次，在屏幕刷新的过程中，如果是PC玩家，换上高刷屏幕以及好一点的显卡，什么都不需要做，因为理论上只要你的屏幕刷新率足够的快，人眼是观察不到画面撕裂的，因为每一帧画面停留在屏幕上的时间太短了，最多会因为显卡输出性能不够，能看到卡顿现象

但显然，不管是LCD还是OLED屏幕，颜色转换总归是需要时间，那么我们就需要思考如何在有限的性能下，减少画面撕裂出现的概率

既然当前的画面撕裂是因为显卡在切换帧缓冲区时，不知道显示器的状态，
...
Android图形系列的Display部分先写到这，后续有新的认知我再回来补充，接下来要开始准备下一篇关于Graphics的文章，敬请期待
