...
在第四章我们分析屏幕刷新率大于视频的帧率的问题时，提到了显示器和视频播放器的各自的工作内容：显示器只管闷头读数据，视频播放器只管写数据
...
如图，系统开机后，显示控制器驱动会按照固定速度读取frame buffer（帧缓存区）的数据显示到屏幕上
...
那就可能发生正在刷新屏幕像素时，帧缓存区的数据被更改，最终导致显示错乱的情况，看图：
...
- **方案二**
  ...
  > 怎么样才能做到呢？
  >
  > 我们可以让显示控制器驱动刷新一幅完整画面之后，发送完成的信号，显卡接收到完成信号后再进行swap buffer，这个完成的信号，就是大名鼎鼎的VSync（垂直同步）信号
  >
  > ps：很多文章和视频都解释画面撕裂因为显卡速度太快导致的，这个说法不严谨，只能说如果帧率明显大于刷新率则会提高看到画面撕裂的概率

方案一实现起来有点困难，因为不管是LCD液晶屏还是OLED屏，颜色转换总归是需要时间的，做不到无限快的速度

那我们只好使用方案二了，这也是现在所有的显卡都支持开启垂直同步选项的原因

但是，我们在玩游戏时，一旦选择开启垂直同步选项，就会觉得游戏画面更卡了，鼠标也不跟手了，这是为什么？

### 3、垂直同步的弊端

开启垂直同步后，显卡就会等VSync信号到来时才会触发交换帧缓存，这就带来两个问题：

- 1、显卡最高输入帧率被限制到屏幕刷新率，实际帧率 = min(刷新率，帧率)
...
- 2、画面延迟
  ...
  > 和问题1一样的原因，接收到VSync信号后交换帧缓存，接着才去绘制新的画面，在等待垂直同步信号的过程中，操作鼠标、键盘的事件虽然被CPU处理了，但因为显卡在阻塞态，没办法立刻渲染出来
  >
  > 这就导致了你看到的画面是有延迟的，比如我向右移动了鼠标，屏幕要过一小会会才会显示出来；在日常办公中这点延迟感受不到，但是在玩游戏时这个延迟就能被感知了
  >
  > 如果是职业玩家以300**APM值**来算，平均每秒会触发5次键鼠事件，他们肯定希望自己的操作能立马呈现到屏幕上
  >
  > 所以，在玩一些实时性要求很高的游戏时，也不建议开启垂直同步选项

综上，我个人是FPS游戏玩家，对于画面的帧率和实时性都有要求，所以垂直同步功能我从来不开

为了解决画面撕裂而发明了垂直同步，现在来看垂直同步带来的两个新问题也比较让人难以接受，付出的代价好像有点大，有没有什么方式能够解决这两个问题呢？
...
>
> 必须要提前说明，三重缓存是在开启垂直同步的情况下才能使用，目的是尽量减少渲染延迟
>
> 开启垂直同步后画面延迟的原因是只有两个frame buffer，一个在供屏幕使用，另一个则已经写好了数据等待交换
>
> 那我们可以再引入一个frame buffer，就叫buffer 3吧，三个buffer组成队列，队列顶部的buffer 1等待VSync信号到来后和队列中buffer 2进行交换
>
> 显卡这边一直在工作，向buffer 3中写数据，一帧绘制完成后和buffer 2进行交换，这样buffer 2永远都是一副完整的帧数据

再说显卡帧率限制为刷新率的问题，这个问题英伟达的G-Sync技术、AMD的Free Sync技术以及苹果的ProMotion自适应刷新率技术都可以解决，这几项技术的本质是让显示器和显卡互相等待

以苹果的ProMoion举例：

> 屏幕不再按照固定的刷新率取数据了，仅保持最低10HZ的速度去帧缓存区读数据
>
> 当应用内容发生变化，会调用显卡进行渲染工作，显卡渲染完成将会通知屏幕等待，接着去交换帧缓存，交换完成后屏幕再去刷新显示
>
> 当显卡输出速度大于屏幕最大值时，13 Pro是120HZ，屏幕会按照最大支持的刷新率来刷新屏幕
>
> 大致流程就是这样，把原来显示器和显卡互不打扰的工作方式，变成了互相等待互相迁就的模式



## 六、结语

在文章的结尾，我们来按照顺序总结一下本章的内容

- 介绍CRT显示器、LCD液晶屏以及OLED屏幕的工作原理，顺便展示了屏幕刷新的过程
- 介绍什么是刷新率的概念，还讨论了刷新率没办法无限提高的原因
- 介绍什么是帧率，分为静态和动态两个概念
- 介绍了什么是画面撕裂以及如何解决
...
哦对了，写文章的过程中我查了非常多的资料，其中有一些我觉得还不错的内容分享给大家，希望这些知识能够帮助到你

- **关于视频帧率，推荐影视飓风团队在B站的视频：[【硬核科普】奇妙的帧率增加了！](https://www.bilibili.com/video/BV1kE411c7yZ)**

  > Tim的视频是我在文章后期才发现的，视频质量很高，因为我本身摄影和后期都会一点，所以原本在逐行/隔行扫描那里加入拍摄前期要注意快门速度和拍摄格式，导出视频是要区分是1080p还是1080i，f原本我在文
