...
屏幕刷新机制涉及到许多技术名词：隔行/逐行扫描、刷新率、帧率、画面撕裂、垂直同步信号、双重缓存等等，每项技术都是为了解决某个问题而生；因此，为了更好的理解每项技术诞生的背景，墙裂建议按照文章顺序阅读本文

overview图片

*ps：本文是笔者作为应用工程师对驱动开发的理解，相关从业者若发现本文的描述错误的地方请及时指出，万分感谢*
...
上图是我自己的设备Pixel 3(OLED屏)，程序中运行的是一个自定义View不停的在蓝色和红色之间来回切换，屏幕会有规律的刷新像素点，至于为什么这样我也没搞清楚，希望业内人士看到了能够指导一二

虽然不清楚原理，但是我觉得这个现象用来解释“逐行扫描”再合适不过了.
...
“隔行扫描”的诞生要追溯到上世纪50年代的CRT时期，由于当时的广播电视传输的带宽不够，所以一幅画面的数据会按照奇偶数分成两次传输，其中奇数行称为上场，偶数行称为下场

显示器每次接收到的都是半幅画面，在PAL制式和NTSC制式中，显示器接受到数据后会先显示奇数行，再显示偶数行，两次显示后凑成一幅完整的图像

由于现在的手机屏幕和电脑显示器几乎都使用的是“逐行扫描”方案，更多关于“隔行扫描”这里也就不展开介绍了

哦对了，有一点需要补充一下，“逐行扫描”的视频后缀用P标识(Progressive)，“隔行扫描”用i标识(Interlaced)，比如1080P意思就是“逐行扫描”的视频，1080i是“隔行扫描”的视频，了解这一点对爱好摄影的小伙伴可能会有所帮助
...
刷新率(Refresh Rate)代表屏幕每秒能够刷新的画面数量
