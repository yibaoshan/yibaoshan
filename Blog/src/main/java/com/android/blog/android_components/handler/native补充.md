...
原因是 Native 层也引入了一套消息管理机制，用于提供给 C/C++ 程序开发者使用

Java 和 Native 都希望 main 线程在没有消息时进入阻塞状态，同时，也希望自己维护的消息队列在有新消息加入时，或者，有到期消息需要执行时，main 线程能及时醒过来执行，而原先的 Object#wait() / notify() 方案是为 Java 层准备的，只支持 Java，现在 Native 也想要，怎么办？有两个方案

要么继续使用 wait() / notify()，让 Native 通知 Java 自己需要什么时候被唤醒

要么在 Native 层实现一套阻塞/唤醒方案，让 Java 通过 jni 调用进入阻塞态

结局我们都知道，Google 选择了后者

其实如果只是将 Java 层的阻塞/唤醒移植到 Native 层，倒也不用祭出 epoll 这个终极大杀器 ，Native 调用 pthread_cond_wait 再通过 jni 开放给 Java 层使用，也能达到相同的效果

选择 epoll 的另一个原因是， Native 层支持监听自定义 Fd （比如 Input 事件就是通过 epoll 监听 socketfd 来实现将事件转发到 APP 进程的），一旦有监听多个流事件的需求，那就只能使用 Linux I/O 多路复用，而 epoll 就是 Linux I/O 多路复用的一个实现
...
接着来讨论 I/O 的操作，通过调用 read() ，我们可以从流中读入数据；通过 write() ，我们可以往流写入数据
...
一个典型的例子为，客户端要从 socket 读数据，但是服务器还没有把数据传回来，这时候该怎么办？

- 阻塞：线程阻塞到 recv() 方法，直到读到数据后再返回
- 非阻塞轮询：recv() 方法没数据立刻返回 -1 ，用户线程使间隔固定时间调用一次 recv() 方法，直到有数据返回

好，现在有了阻塞和非阻塞两种解决方案，接着我们同时发起100个网络请求，这两种方案各自会怎么处理呢？
...
这种能够让用户程序拥有同时监听多个流读写事件的机制，就是所谓的 I/O 多路复用！
...
最后一个 epoll_wait() 方法是使 Java 和 Native 的消息队列都能阻塞的方法，它接受一个事件集合 events ，如果有多个事件同时发生，我们可以在 events 集合中读取，最后一个 timeout 参数表示超时时间
