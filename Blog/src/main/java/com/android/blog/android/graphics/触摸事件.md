...
class ViewRootImpl {
    ...
    void setView(){
        mInputChannel = new InputChannel(); // 创建了空的 InputChannel
      	try {
          	Session.addToDisplay(mInputChannel);
...
中间的传递过程我们先不管，反正 mInputChannel 变量不为空，就表示应用端的 socket 已经传回来了

### 3、sockets[0] 传递到 IMS

 

本小节想要聊的是， “ APP 是如何和 IMS 建立通信的 ”。在建立通信的过程中，可能会涉及到一些我们没见过的类、没用过的系统机制，届时我会简单介绍一两句，实现原理我们可以先不深究

建立通信这部分内容，和前几节没有任何联系，可以单独看做一篇文章。因此，暂时没理解某个环节影响也不大，我们只需要记住，在窗口（Activity、Dialog）创建的时候，顺便创建了和 IMS 通信的通道就行了
