...
Android Input 事件的类型，和分发的流程都比较复杂，除了触摸事件外，系统还有来自鼠标、键盘、音量键、电源键等其他设备的 Input 事件需要处理

我们日常开发接触比较多的 Input 是 ‘触摸事件’，因此，本文主要讨论的是 ‘触摸事件’ 的分发流程（*其他事件路径也差不多*）
...
在[《当我们点击“微信”应用后，它是怎么显示出来的？》](https://mp.weixin.qq.com/s/JeGPyknzc0G_TCQNHZD3AQ)这篇文章中，为了搞清楚什么是 HWC，我们拆了一台小米11手机
...
如上图，这是拆解后的小米10内部布局，图中左边黄色箭头所指的部分，触摸屏的触控芯片

米10的触控芯片使用的是，来自意法半导体"FJABH"触控芯片方案，这块芯片是用来干嘛的呢？

用来和 CPU 进行通信的

我们知道，当我们按下触摸屏后，屏幕的电压/电流大小会发生变化（不讨论触摸屏工作原理）

变化的电压/电流会被触控 IC 捕获，接着计算出触摸位置的坐标值，通过 I²C 总线发送到主板上的 CPU

我是图片

[ I²C ](https://elixir.bootlin.com/linux/v4.4.1/source/drivers/i2c)是硬件之间常用的一种通信协议，规定了什么表示起始、停止、应答和非应答等一系列信号

作为应用开发，我们不需要关心他们之间的通信细节

我们只需要知道： "只要触摸屏的信号发生变化，触控芯片就能通过 I²C 总线通知到 CPU "，这一点就够了

好了，这下触摸信号已经能够被 CPU 读取了，接下来我们看 CPU ，也就是操作系统怎么处理
